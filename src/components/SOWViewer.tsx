...
export default function SOWViewer({ text, streaming = false, hideCost = false, contractorView = false }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {!streaming && (
        <div className="mt-6 pt-4 flex items-center justify-between" style={{ borderTop: '1px solid oklch(0.20 0.022 252)' }}>
          <p className="text-xs" style={{ color: 'oklch(0.35 0.015 252)' }}>
            © {new Date().getFullYear()} TAGS — The Apartment Guys System. All rights reserved.
          </p>
          <p className="text-xs" style={{ color: 'oklch(0.35 0.015 252)' }}>
            Generated by TAGS · tagyourproject.com
          </p>
        </div>
      )}
    </div>
